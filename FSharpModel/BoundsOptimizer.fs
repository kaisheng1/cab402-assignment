...
let unitDependenciesWithinPlan (allUnits: StudyPlan) : seq<UnitCode*UnitCode> =
    ...
    let pairsWithCurrent (current: UnitInPlan) (withoutCurrent: StudyPlan) (isNotEnrollableAfterRemoveCurrent: UnitCode->bool): seq<UnitCode*UnitCode> = 
        let addPairs acc (elem: UnitInPlan) = 
            if (isNotEnrollableAfterRemoveCurrent elem.code) then Seq.append (Seq.singleton (current.code, elem.code)) acc
            else acc

        Seq.fold addPairs Seq.empty withoutCurrent
        
    let allPairs acc (elem: UnitInPlan) = 
        let withoutCurrent = Seq.filter (fun (x:UnitInPlan) -> x.code <> elem.code) allUnits
        let isNotEnrollableAfterRemoveCurrent y = not (isEnrollable y withoutCurrent) //reuse the withoutCurrent

        Seq.append (pairsWithCurrent elem withoutCurrent isNotEnrollableAfterRemoveCurrent) acc

    Seq.fold allPairs Seq.empty allUnits
...
let rec private firstOfferingOnOrAfter (unitCode: UnitCode) (semester: Semester) : Semester =
    ...
    if (isOffered unitCode semester) then semester
    else firstOfferingOnOrAfter unitCode (nextSemester semester)
...
let rec private firstOfferingOnOrBefore (unitCode: UnitCode) (semester: Semester) : Semester =
    // TODO: Fixme (difficulty: 3/10)
    if (isOffered unitCode semester) then semester
    else firstOfferingOnOrBefore unitCode (previousSemester semester)
...
let rec private earliestSemester (dependencies: seq<UnitCode*UnitCode>) (unitCode: UnitCode) (firstSemester: Semester)  : Semester =
    ...
    let earliest = firstOfferingOnOrAfter unitCode firstSemester
    let dependenciesOfUnit = Seq.filter (fun (x, y) -> y = unitCode) dependencies //find if it's dependent on other units

    //return the first offered unit after firstSemester if not found
    if (Seq.isEmpty dependenciesOfUnit) then earliest
    else 
        let dependencyEarliest x = earliestSemester dependencies x firstSemester
        Seq.fold (fun acc (x,y) -> 
            let dEarliest = dependencyEarliest x
            if (dEarliest < acc) then acc 
            else firstOfferingOnOrAfter y (nextSemester dEarliest)
        ) earliest dependenciesOfUnit //it should always be later than its dependencies
...
let rec private latestSemester (dependencies: seq<UnitCode*UnitCode>) (unitCode: UnitCode) (lastSemester: Semester) : Semester =
    // TODO: Fixme (difficulty: 9/10)
    let latest = firstOfferingOnOrBefore unitCode lastSemester
    let subordinatesOfUnit = Seq.filter (fun (x, y) -> x = unitCode) dependencies //find those units that are dependent on this unit

    if (Seq.isEmpty subordinatesOfUnit) then latest
    else 
        let subordinateLatest y = latestSemester dependencies y lastSemester
        Seq.fold (fun acc (x,y) -> 
            let sLatest = subordinateLatest y
            if (sLatest > acc) then acc 
            else firstOfferingOnOrBefore x (previousSemester sLatest)
        ) latest subordinatesOfUnit //it should always be earlier than its subordinates
...
let boundUnitsInPlan (allUnits: StudyPlan) (firstSemester: Semester) (lastSemester: Semester) : BoundPlan =
    ...
    let unitDependencies = unitDependenciesWithinPlan allUnits

    let possibleSemesters unitCode firstSemester lastSemester =  
        let earliestSem = earliestSemester unitDependencies unitCode firstSemester
        let latestSem = latestSemester unitDependencies unitCode lastSemester
        let rec allOfferedSemesters (current: Semester) (latest: Semester) = 
            if (current = latest) then seq {latest}
            else if (current > latest) then Seq.empty
            else 
                let nextOffering semester = firstOfferingOnOrAfter unitCode (nextSemester semester)
                seq {
                    yield current 
                    yield! allOfferedSemesters (nextOffering current) latest
                }

        allOfferedSemesters earliestSem latestSem


    allUnits
    |> Seq.map (fun unit -> { code =unit.code; studyArea = unit.studyArea; possibleSemesters = (possibleSemesters unit.code firstSemester lastSemester)}) 
    |> Seq.toList